...
## 8. 第二轮：组件级重设（2026-09-11 用户验收反馈后）

第一轮以"令牌驱动换肤"为主，用户验收后指出三处问题与一个方向性问题。第二轮逐条处理，并把"组件重设"落实到形态层（不再只是换色）。

**a. 按钮回退浏览器默认外观（真实缺陷，已修）**
第一轮清扫时把 `.evo-btn` 的 `border: none` 也删掉了，而组件层基础态又没有补 `background/border`，于是 `.evo-btn`（如设置面板"选择文件夹"）掉回浏览器默认外观——实测计算样式为 `border: 2px rgb(0,0,0)` + `background: rgb(240,240,240)`，看起来像"单独写的样式"。
修复：组件层 `.evo-btn` 基础态改为**完整的次级按钮样式**（surface 底 + 1px 暖描边 + 鹅卵石圆角 + 轻投影），变体只覆盖语义色；并新增守卫断言——`.evo-btn` 分组必须声明 `background:` 与 `border:`，否则构建失败。
验收：浏览器遍历全部可见按钮（欢迎页 76 / 会话 57 / 设置 63），**0 个**命中 UA 默认特征（2px 边框 / `#f0f0f0` 底）。

**b. 模型胶囊里的"数字"（不是数据 bug，是展示 bug，已修）**
`262144` 是上下文窗口输入框的 **placeholder**（提示"留空用默认值"），不是每个模型的真实值，但裸数字看起来就是一个错误数字。
修复：改为**人去化只读胶囊** —— 未设置时显示虚框"默认 262K"，已设置时显示实线鼠尾草胶囊"128K"（精确值在 tooltip，进入编辑才显示数字输入框）。交互实测：点击胶囊 → 出现 number 输入框 → 输入 128000 → 回车 → 胶囊变"128K"。
同时把 `262144` 抽成 `CONTEXT_WINDOW_FALLBACK` 常量，说明文案不再把裸数字当"值"。

**c. hover 改变圆角（风格文档要求，但实际读起来像 bug，已废弃）**
风格提示词里的 Organic Morphing 明确要求"不规则圆角在交互中缓慢变化"，第一轮据此让可交互卡片 hover 时从 `--r-blob` 形变到 `--r-blob-lg`（如"清除数据"行 22/17/24/15 → 34/26/36/22）。用户反馈像渲染抖动。
处理：**移除全部 hover 圆角形变**，改为"描边返青 + 上浮 1px + 阴影加深"；静态形状仍保留不对称 blob/鹅卵石圆角。守卫新增断言：任何 `:hover` 规则内出现 `border-radius` 即失败。
实测：`.evo-clear-row` hover 前后圆角稳定为 `22px 17px 24px 15px`。

**d. "只是换了个颜色主题"——组件形态重设**
第二轮把设计语言落到形态上（仍全部走令牌与组件层）：
- **按钮**：pill → **鹅卵石**（`--r-pebble` 15/12/16/13，小号 `--r-pebble-sm` 10/8/11/9，四角不等）；主按钮加"顶部一线内高光"（手工漆面），语义按钮改"浆染"底 + 同色描边，新增 `.evo-btn-ghost`。
- **徽标/胶囊**：pill → 鹅卵石 + 同色描边；新增 `-dashed` 变体表达"未设置/默认值"（形态区分而非只靠颜色）。
- **输入控件**：加极浅内阴影（凿面感），hover 描边返青，聚焦双层环。
- **卡片**：blob 圆角 + 纸片投影；hover 只上浮不形变。
- **浮层**：`--r-md` → `--r-blob` 不对称圆角 + 顶部一线内高光（纸边）。
- **消息气泡**：叶尖圆角（AI `6px 20px 20px 20px`、用户 `20px 6px 20px 20px`）+ 轻纸影。
- **图标钮家族**（22 条规则）：小圆角统一成鹅卵石小圆角。
- **页签/分段控件**：选中态改为鼠尾草"叶脉"短条（视图页签在底部、编辑器页签在顶部），分段控件改"凹槽轨道 + 鹅卵石滑块"。
- **图形签名**：新增 `--sprig` 植物小枝（mask 上色）——欢迎页水印、面板标题前、纯文字空态；AI 头像由图标改为**幼苗**（同一支小枝）；会话标签盘与 favicon 已在第一轮换成大地色。
- 已有图标语义的空态不叠小枝（`:has(svg)` 排除），避免两个图形元素打架。

**e. 顺带修掉的两个真实缺陷**
1. **图谱缩放倒角三角与卡片圆角脱钩**：第一轮把 `--graph-node-radius` 从 16px 改成 20px 时，四个 `border-*-radius: 16px` 硬编码没跟着改，缩放提示三角与卡片边缘错位。由新增的"禁裸圆角字面量"守卫发现，已改为 `var(--graph-node-radius)`。
2. **主题切换按钮可能失效（pre-existing）**：`clientStateHydrate` 回填偏好后会 `applyTheme()` 重刷 `html.dark`，但 React 的 `themeDark` 不同步——顶部按钮的图标/提示停留在旧值，且"按偏好取反"的切换会点一下没反应（偏好与被回填后的已应用主题同向）。修复：订阅既有的 `evo:client-state-loaded` 事件同步 `themeDark`。实测连续点击三次：dark→true→false→true，DOM/偏好/按钮标题三者始终一致。

**f. 新增/扩展的守卫断言**
- 必需令牌增加 `--r-pebble-sm` / `--r-pebble` / `--r-bubble-ai` / `--r-bubble-user` / `--sprig`；
- `.evo-btn` 基础态必须自带 `background` + `border`（防 UA 默认外观回归）；
- 禁止 `:hover` 规则改圆角；
- 令牌块之外禁止裸圆角字面量（豁免 `evo-modal-full` 与 `pre code`）。
四项断言均做过自检（注入违规样本能正确失败）。
...
## 10. 第四轮：深色层次重标定（用户反馈"两个颜色挨太近、看不出是两个颜色"）

**问题**：深色主题下相邻面差得太小。实测标定（CIE Lab 明度）确认了这一点：

| 层级 | 改前 | L* | 相邻 ΔL* |
|------|------|----|---------|
| 页面底 | `#1e1b16` | 9.9 | — |
| 卡片面 | `#292420` | 14.6 | 4.7（ΔE00 仅 3.4） |
| 悬停面 | `#352d24` | 19.1 | 4.5 |
| 页签条（现算） | ≈`#282522` | 13.5 | **3.6（与页面几乎同色）** |
| 输入面 | `#17140f` | 6.5 | 3.5 |

大色块并排时 ΔE00 ≈ 3 已读作"同一个颜色"，而深色下阴影几乎不可见、只剩这点明度差 → 用户说"看不出是两个颜色"。

**方法与结论**：按 CIE Lab 重排明度阶梯（约束色相 68–88° 的暖褐族，色度随高度递增），把"相邻面 ΔL*"作为一等约束：

| 层级 | 改后 | L* | 相邻 ΔL* |
|------|------|----|---------|
| 井：页签条 / 图谱画布 | `#151009` | 5.0 | — |
| 凹陷：输入面 | `#151009` | 5.0 | — |
| 页面底 / 栏底 | `#231f19` | 12.0 | **7.0（井 → 页面）** |
| 卡片 / 面板 / 气泡 | `#302922` | 17.1 | **5.1（页面 → 卡片）** |
| 抬起：浮层 / 模态 / 图谱控件 | `#3c332b` | 22.0 | **4.9（卡片 → 浮层）** |
| 描边（轻 / 常规） | `#3d342c` / `#4a4237` | 22.4 / 28.5 | — |

要点：
1. **浮动层独立成一级**：新增 `--surface-raised`（弹层/模态/toast/图谱控件），此前它们与卡片同色，浮起后仍"糊"在卡片里；
2. **输入面沉到"井"级**：输入框与页面/卡片的明度差从 3.5 拉到 7.0+，字段读作"凿出来的槽"；
3. **页签条沉到井级**：条比内容面深 7 个 L*，"活动页签 = 内容面色"才真正读作与内容融接（Chromium 的条/页关系）；
4. **悬停改为半透明暖洗**（`color-mix(text-primary 7%, transparent)`）：叠在任意父面上都成立，不会在小卡片里突然变浅；
5. **文字阶梯同步提亮**：表面抬亮后，原有的 `#9d9182` 三类文字在新表面上掉到 4.0 以下（实测 tertiary on raised = 3.43），故文本改为
   `primary #f2ece1 / secondary #d6cdbc / tertiary #bfb4a5`（层级 ΔL* 仍 ≥ 10），语义色同步提亮
   （success `#a9c886`、warning `#e3bd6f`、error `#eda78f`、info `#9eccbe`）；
6. **浅色主题不动**（用户认可）：浅色的分层由阴影 + 描边承担，只补了 `--strip-bg: #f0e9dd`（条比页面深 4.5 L*）与把悬停面上不达标的墨色加深一档
   （tertiary `#6e6355`、success `#526b3d`、warning `#895c18`、info `#416d63`、tan-ink `#845b29`），使"悬停面"也成为受检表面。

**新增守卫（把可辨性变成可失败断言）**
- ⑤ 层次阶梯：深色 `页面→卡片 ≥ 4.5`、`卡片→浮层 ≥ 4.5`、`页面→页签条 ≥ 4.5`、`输入→卡片 ≥ 4.5`；浅色 `页面→页签条 ≥ 3.5`、`悬停→页面 ≥ 3.5`；任一不足即构建失败（已做注入自检：把卡片面改成接近页面色 → 报 `ΔL*=1.0（需 ≥ 4.5）`）。
- ④ 对比度校验从 36 对扩到 **98 对**：主/次/三/占位/五个语义色 × {页面, 卡片, 浮层} + 半透明悬停洗色的合成面；并把色值解析扩展到 `color-mix(...)` 表达式（按"叠在最亮实心面"的最坏情况合成）。

**端到端实测（截图取样的真实像素）**：页签条 `#19150e`（L*7.1，含纸纹）／页面 `#26221c`（13.8）／输入容器（卡片）`#302922`（17.1）／浮层 `#4c433a`（29，含阴影边缘）／设置模态 `#362f27`（19.9）对其内凹卡片 `#231f19`（12.0）→ 全部为可辨层级；0 console 报错。
截图：`.tmp-dev/images/organic/D1–D4`（深色四层 + 设置分层 + 取样探针）、`E1/E2`（浅色复核）。

**h. 页签的尺寸与选中表达（第五轮，用户验收反馈）**
用户指出三点：① 页签太窄、文字与边框的内边距不足；② 顶边那道绿色高亮不喜欢（并质疑风格文档是否要求横线高亮）；③ 关闭钮紧贴标题、横向占比近 1/3，容易误点。
处理：
- **撤掉绿色顶线**。风格文档只要求"柔和过渡 / 大地色 / 有机形状"，并未要求用彩色横线表达选中；那道鼠尾草线是我自创的"标签组色线"，读起来像一条绿色高亮，已移除。选中态改为 Chromium 的原生语言：**面（= 内容面色）+ 暖色描边 + 字重 600**，不加任何彩色标记。检查器页签的绿色下划线同源问题一并去掉，改为"浅底 + 字重"（与顶部页签同一语言）。
- **给页签定最小宽度与呼吸位**：`min-width: 138px`、`max-width: 224px`、`height: 34px`、内边距 `0 8px 0 16px`、内部 `gap: 10px`；标题 `flex: 1 1 auto` 左对齐省略，关闭钮 `margin-left: auto` 右对齐。
- **关闭钮不再咬字**：命中区外扩从 6px 收到 3px（此前外扩区会盖住标题尾部），并让它右对齐。
实测（计算样式 + 命中测试）：页签宽 138px、标题左起 17px、关闭钮距右 9px、**标题与关闭钮净间距 10px**、关闭钮只占页签宽度 **13%**（原约 33%）；标题右侧 6px 处点击命中 `DIV.evo-tab`（不再命中关闭钮），关闭钮中心命中 `BUTTON.evo-tab-close`。
截图：`.tmp-dev/images/organic/F1`（浅色）/ `F2`（深色）。
