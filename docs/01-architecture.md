...
## 五类资料的职责边界（原文 / 索引 / 笔记 / Chat Graph / 上下文构造）

这里的“五类”是系统边界，而不是用户需要填写的五种表单：原文是唯一主资料，索引是可删除重建的定位视图，自然语言笔记是可编辑导航，Chat Graph 是研究者显式信号，ContextAssembler 是本轮临时投影。五者之间只有原文拥有事实权威；任何摘要、连接、缓存或布局结果都必须能够回到原文或安全地失效。

系统里同时存在五类资料，它们职责不同、生命周期不同，不能互相替代：

```mermaid
flowchart LR
    RAW["原文<br/>（会话原文·工具输出·文件·实验日志）"] -->|"不可替代的主资料"| ASSEMBLE
    RAW -->|"可重建的派生视图"| INDEX["索引<br/>（FTS5 / 向量 / 片段定位）"]
    NOTES["自然语言笔记<br/>（自由文本 Markdown）"] --> INDEX
    GRAPH["Chat Graph<br/>（研究者显式连线信号）"] --> ASSEMBLE
    INDEX -->|"候选定位"| ASSEMBLE["上下文构造<br/>（ContextAssembler）"]
    ASSEMBLE -->|"按当前问题临时组装"| AGENT["模型调用上下文"]
    AGENT -->|"回答与产物"| RAW
    NOTES -.->|"研究者可随时阅读/编辑"| RAW
    GRAPH -.->|"分支=新的原文会话"| RAW
```
...
| 资料 | 职责 | 边界（不是什么） |
|---|---|---|
| **原文（Raw）** | 唯一主资料：完整保存对话原文、工具调用与结果、文件、实验日志、LaTeX 稿件 | 不因“已归档/已摘要”而删除或只剩摘要；摘要只用于定位，不替代原文 |
| **索引（Index）** | 从原文派生的可重建视图：FTS5 全文、向量、片段定位（保存回到原文的位置） | 不是主资料；库损坏或迁移后可整体重建，重建只损失检索效率，不损失内容 |
| **自然语言笔记（Notes）** | 研究者的自由文本：Idea、方法变化、实验理解、研究近况，长短不限 | 不要求字段/来源/分类/置信度；笔记正文是人的表达，系统只负责索引与检索，不强行结构化 |
| **Chat Graph（Graph）** | 研究者的显式信号：context 连线=分支从哪段讨论而来，reference/relation 分别控制持续参考与普通关系；资源节点只保存 locator | 不是自动生成的推理结果，也不是唯一上下文来源；图只描述关系，不保存正文副本 |
| **上下文构造（ContextAssembler）** | 按当前问题临时组装：先经索引定位候选，再回读原文前后文，叠加 Graph 显式信号与笔记，并输出 Memory→原文的 Context Trace | 组装结果是临时的模型输入投影，不落盘为“权威记忆”；每次问题都可以不同 |

关键不变式：

1. **原文 → 索引** 是单向派生，索引永远可重建；任何“重建索引”操作不得触碰原文。
2. **Graph 与笔记只指向原文**（会话、轮次、文件路径），不复制正文，避免两处修改不同步。
3. **上下文构造是只读组装**：只读取原文/索引/笔记/图，不修改它们；写回只发生在研究者主动操作（新建笔记、改连线、开分支）。
4. **分支会话本身是新原文**：从某轮 fork 出的新会话一旦产生消息，就独立成为原文的一部分，不再依赖来源会话的投影。
