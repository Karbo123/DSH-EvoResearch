...
┌────────────────────────────────────────────────────────────────────┐
│                        浏览器（DSH Web GUI）                         │
│  dsh-client-ui-*（平台）   +   @evoscientist/dsh-plugin/client     │
│    侧栏「🔬 科研」入口 / 科研面板(overlay) / 会话记忆提示条(dock)     │
│    └── ctx.remote.evosci.*（Typert Gateway / JSON RPC）            │
└──────────────────────────────┬─────────────────────────────────────┘
                               │
┌──────────────────────────────▼─────────────────────────────────────┐
│                      Host（Node.js 进程）                           │
│  @evoscientist/dsh-plugin/host ── 插件入口（apply 组装）             │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ WorkspaceService   科研项目工作区（projects/<n>/.evosci-data） │  │
...
│  │ EvosciApiService  Typert Remote API（evosci.*）                │  │
...
| `api.ts` | EvosciApiService（Typert Remote，Client 可调） | `langgraph_dev/http.py` |
| `commands.ts` | 斜杠命令注册（平台命令体系） | `commands/implementation/*` |
| `index.ts` | 插件入口：配置解析 + 组装 + 副作用管理 | `EvoScientist.py` 接线层 |

### Client 插件（`src/client/`）

| Slot | 用途 | 对应 EvoScientist WebUI |
|---|---|---|
...
MemoryRuntime.handleSessionEvent
  ...
  2. 后台 processTurnBackground：
     ...
     e. buildMemoryPacket(query=用户消息) → packets[sessionId]
              │
              ▼
systemPrompt.context('evosci:research-memory')
...
├── projects/
│   └── <name>/                    # 独立 git 仓库
│       ├── .evosci-data/          # git exclude（info/exclude）
│       │   ├── memories/
...
- `read_memory` 工具限制在 `.evosci-data/memories/` 内；
