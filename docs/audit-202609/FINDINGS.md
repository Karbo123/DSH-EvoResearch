...
## 四·七、视觉审查战役（2026-09-05/06：合并后 main 的全界面×多视口复查）

合并 v4 图谱后对 main 做了全面视觉复查：**17 档视口**（1920/1600/1440/1366/1280/1152/1024/900/820/768/600/480/414/390/375/360/320）× **16 个界面**（首页/打开会话/九视图/检查器三 tab/设置三 tab/模型菜单）= **272 组合**，程序化检测横向溢出/元素越界/过小按钮 + 全量截图目检。

### 发现并修复（第六·五批）
1. **平板竖屏断点**：768/820 宽度仍用桌面双栏，聊天区被挤到 ~300px——抽屉断点 767→819（CSS）/768→820（JS），平板竖屏改抽屉模式。
2. **composer Markdown 工具条**：≤900 溢出视口且不可达（overflow:hidden 裁切）——改 overflow-x:auto 可横滚（设计同移动端 chip 行）。
3. **检查器子标签行**：768 刷新/下载钮被截断（+59px 溢出）——行改 flex-wrap。
4. **模型设置 API Key**：375 明文回显整串密钥——两处输入改 type:password（web-search 面板早已是 password，模型面板漏了）。
5. **llm 表单字段**：url 360px/key 460px 固定不收缩，≤600 溢出视口——改 `flex: 1 1 320px; max-width: …`。
6. **模型分配三列网格**：固定 490px（150+220+120），≤600 溢出——列改 minmax 弹性 + ≤600 单列堆叠。
7. **标签关闭钮**：17px 热区过小——::after 扩展命中区至 ~29px（视觉不变）。

### 复检结果
- 768/820/900 的 ELS-OUT、inspector +59px 溢出、llm 表单溢出：**全部消除**。
- 残余两类均非问题：md 工具条按钮在窄屏位于横滚区内部（可滚动可达，设计如此）；tab 关闭钮 rect 17px 但 ::after 热区已扩展（检测器量 rect 的假阳性）。
- 抽查截图目检（1920/768/820/375 × 首页/会话/检查器/设置模型）：布局无破裂、无遮挡、信息密度正常。

**基线**：单测 646+52+5 全过、typecheck 0 错误、build 0 警告、check-docs 通过。
