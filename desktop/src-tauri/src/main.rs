//! EvoResearch 桌面壳：spawn Node sidecar（DSH web 服务）→ 读取端口 → WebView 加载。
//!
//! 协作协议（desktop：Windows / Linux / macOS）：
//! 1. 壳在资源目录中定位 sidecar（node(.exe) + app/ + launch.js，由 bundle-sidecar.mjs 组装）；
//! 2. spawn `node launch.js`（Windows 隐藏控制台）；
//! 3. launch.js 启动 DSH web profile（EvoResearch）后，把端口写入端口文件
//!    （平台本地数据目录/com.evoresearch.desktop/port.json）并打印一行 JSON 到 stdout；
//! 4. 壳轮询端口文件（≤60s）后加载 `http://127.0.0.1:<port>`；
//! 5. 壳退出时终止 sidecar 进程树（Node 侧 process.on('exit') 兜底）。
//!
//! mobile（Android/iOS）：无 Node sidecar（DSH 后端不支持移动端运行时），
//! 直接加载打包进二进制的占位页（tauri.android.conf.json 清空 resources）。
...
/// 桌面入口（Windows / Linux / macOS）：spawn sidecar → 等端口 → 加载 WebUI。
#[cfg(not(target_os = "android"))]
fn desktop_main() {
