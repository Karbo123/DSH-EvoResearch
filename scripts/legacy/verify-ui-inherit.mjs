...
await sleep(2500)
// 找"研究起点"（A=21deb9ce）与目标 chat 节点（B=5d95be06，恢复 A→B 上下文边）
const nodes = await evalJs(`(() => [...document.querySelectorAll('.evo-graph-node-chat')].map(n => {
  const out = n.querySelector('.evo-graph-socket-out').getBoundingClientRect()
  const inCtx = n.querySelector('.evo-graph-socket-in.evo-graph-socket-ctx').getBoundingClientRect()
  return {
    ...
    inCtx: { x: Math.round(inCtx.left + inCtx.width / 2), y: Math.round(inCtx.top + inCtx.height / 2) },
  }
}))()`)
console.log('CHAT NODES:', JSON.stringify(nodes.map((n) => ({ id: n.id, title: n.title }))))
const src = nodes.find((n) => n.id === '21deb9ce') ?? nodes.find((n) => n.title.includes('研究起点')) ?? nodes[0]
const dst = nodes.find((n) => n.id === '5d95be06') ?? nodes.find((n) => n.id !== src.id)
if (src && dst) {
...
const g = (await (await fetch(`http://127.0.0.1:${PORT}/evoresearch/fs/graph-get`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ workspaceDir: wsDir }) })).json()).value.graph
