/**
 * Round 38 验证：§26.5 Identity 记忆文件展示。
 * 1. 预置 profile 文件（SOUL.md）；
 * 2. 打开 EvoMemory → Identity tab → 文件卡片 + 内容预览；
 * 3. 截图。
 * 用法：node scripts/verify-identity.mjs <port>
 */
...
const debugPort = 39000 + Math.floor(Math.random() * 900)
const userData = join(ROOT, '.tmp-port', `edge-id-${randomBytes(4).toString('hex')}`)
...
async function main() {
  ...
  const out = join(ROOT, '.tmp-port', `identity-${port}.png`)
