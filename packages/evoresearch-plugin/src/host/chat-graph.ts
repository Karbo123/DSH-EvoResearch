...
export class ChatGraphService {
  ...
  save(projectName: string, graph: ChatGraph, expectedRev?: number): { ok: boolean; error?: string; conflict?: boolean } {
    ...
    try {
      ...
      fs.writeFileSync(tmp, JSON.stringify({ nodes: projectNodes, edges: deduped, groups: normalized.groups, schemaVersion: normalized.schemaVersion, rev: nextRev }, null, 2), 'utf8')
