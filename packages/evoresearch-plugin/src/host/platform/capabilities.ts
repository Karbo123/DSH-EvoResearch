...
export const DSH_CAPABILITY_MATRIX: readonly CapabilityPlan[] = [
  ...
  },
  {
    ...
    status: 'partial',
    layer: 'dsh',
    serviceNames: ['sandbox', 'sandboxPolicy'],
    basis: '0.1.3 起有正式 ctx.sandbox（同世界 OS 级沙箱接缝：Linux bwrap/Landlock、macOS Seatbelt、Windows ACL 受限令牌；SandboxMode = read-only/workspace-write/danger-full-access，仅文件效果，fail-closed SANDBOX_UNAVAILABLE）。容器/微虚拟机/远程执行不是该接缝后端——需整体替换 ctx.shell/ctx.fs。rc.6 时代仅 tools.restrict/guard。',
    adapter: 'shell/pwsh 工具经 preset 行消费 host 的 bash-sandbox/pwsh-sandbox；工具级隔离保留 adapters.sandbox（restrict/guard 包装）；文件系统隔离由 WorkspaceService 路径校验与项目 .evoresearch-data 承担。',
    degradation: '无沙箱后端 → shell 调用按 SANDBOX_UNAVAILABLE 显式报错（不裸跑）；文件隔离仍由工作区路径校验保证。',
  },
  {
    capability: 'events',
    status: 'available',
