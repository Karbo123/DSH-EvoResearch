/**
 * EvosciApiService 单元测试：验证 Typert Remote 服务在最小 cordis 上下文中
 * 可构造（服务 + gateway 绑定注册无冲突），且 @Remote 装饰器标记的方法
 * 能被 remoteMethods 发现（wire 可调用 —— 这是浏览器端 ctx.remote.evosci.*
...
import { EvosciApiService, type HostServices } from '../src/host/api.js'
...
describe('EvosciApiService', () => {
  it('可在最小 cordis 上下文构造（服务 + gateway 绑定注册）', () => {
    const ctx = new Context()
    const service = new EvosciApiService(ctx, fakeServices())
    assert.ok(service)
    ...
    assert.equal(binding?.serviceKey, 'evosci')
    assert.equal(binding?.namespace, 'evosci')
  })
  ...
  it('Remote 方法调用返回 JSON 结果（projectsList 空列表）', async () => {
    ...
    const dataRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'evosci-api-'))
    const workspace = new WorkspaceService({ dataRoot })
    const service = new EvosciApiService(ctx, {
      workspace,
