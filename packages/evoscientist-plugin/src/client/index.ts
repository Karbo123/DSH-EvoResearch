 ...
 * 不持有任何 Host 内部对象。第一版样式使用内联 style + DSH 主题 CSS 变量。
 */
...
/** 科研面板（overlay）。 */
function ResearchPanel({ ctx, onClose }: { ctx: Context; onClose: () => void }) {
  ...
  const body = (): React.ReactElement => {
    const list = (items: unknown[], render: (item: unknown, index: number) => React.ReactElement): React.ReactElement =>
      items.length === 0
        ? React.createElement('div', { style: { color: 'var(--dsw-alias-label-caption, #999)' } }, '（空）')
    ...
    if (tab === 'memory') {
      return React.createElement(
        React.Fragment,
        null,
        React.createElement('div', { style: { fontWeight: 600, marginBottom: 8 } }, '科研记忆类别目录'),
    ...
    if (tab === 'scheduler') {
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement('input', { placeholder: 'cron（如 0 9 * * 1-5）', value: newCron, onChange: (e) => setNewCron(e.target.value), style: { flex: 1, minWidth: 160, padding: '6px 10px', borderRadius: 6, border: '1px solid var(--dsw-alias-border-l2, #ddd)' } }),
          React.createElement('input', { placeholder: '任务提示词', value: newPrompt, onChange: (e) => setNewPrompt(e.target.value), style: { flex: 2, minWidth: 240, padding: '6px 10px', borderRadius: 6, border: '1px solid var(--dsw-alias-border-l2, #ddd)' } }),
          React.createElement(
            'button',
        ...
        list(tasks, (task, index) => {
          const t = task as { taskId?: string; cron?: string; name?: string; enabled?: boolean }
          return React.createElement('div', { key: index, style: rowStyle }, React.createElement('span', null, `${t.cron ?? ''} ${t.name ?? ''}`), React.createElement('span', { style: { color: 'var(--dsw-alias-label-caption, #999)' } }, t.enabled ? '开' : '关'))
        }),
      )
    }
    if (tab === 'channels') {
      return list(channels, (channel, index) => {
        ...
        return React.createElement(
          ...
          React.createElement('span', { style: { color: 'var(--dsw-alias-label-caption, #999)' } }, `收${c.received ?? 0} 发${c.sent ?? 0}`),
          React.createElement(
            'button',
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { style: headerStyle },
      React.createElement('strong', null, 'EvoScientist 科研面板'),
      React.createElement(
        'button',
    ...
    React.createElement('div', { style: tabsStyle }, tabButton('projects', '项目'), tabButton('memory', '记忆'), tabButton('scheduler', '定时任务'), tabButton('channels', '通道'), tabButton('autoskills', '技能提案')),
    React.createElement('div', { style: bodyStyle }, body()),
  )
}

/** 记忆提示条（composer 上方 dock）。 */
function MemoryDock({ sessionId, ctx }: { sessionId: string; ctx: Context }) {
  ...
  React.useEffect(() => {
    ...
    void evosci.memoryPacket({ sessionId }).then((value) => {
      ...
      if (packet) {
        ...
        setSummary(`科研记忆：${states} 个主题 · ${hits} 条相关历史`)
      } else {
        setSummary('')
      }
    })
  ...
  return React.createElement(
    'div',
    { style: { fontSize: 12, color: 'var(--dsw-alias-label-tertiary, #888)', padding: '0 4px 4px', cursor: 'pointer' }, onClick: openResearchPanel, title: '打开科研面板' },
    `🧠 ${summary}`,
  )
}

/** 侧栏底部入口按钮。 */
function SidebarEntry({ onClick }: { onClick: () => void }): React.ReactElement {
  return React.createElement(
    'button',
    {
      onClick,
      ...
      title: '科研项目 / 记忆 / 定时任务 / 通道 / 技能提案',
    },
    '🔬 科研',
  )
}

/** Client 插件 apply：注册三个 Slot。 */
export function apply(ctx: Context): void {
  ...
  slots.inject('sidebar.footer.action', () =>
    slots.register(
      { name: 'sidebar.footer.action', id: 'evosci-research', order: 90 },
      () => {
        ...
        return React.createElement(
          React.Fragment,
          null,
          React.createElement(SidebarEntry, { onClick: () => setOpen(true) }),
          open ? React.createElement(ResearchPanel, { ctx, onClose: () => setOpen(false) }) : null,
        )
      },
    ),
  )
  ...
  slots.inject('shell.overlay', () =>
    slots.register(
      { name: 'shell.overlay', id: 'evosci-research-panel', order: 100 },
      () => {
        const [open, setOpen] = React.useState(false)
        React.useEffect(() => {
          const listener = (): void => setOpen(true)
        ...
        return open ? React.createElement(ResearchPanel, { ctx, onClose: () => setOpen(false) }) : null
      },
    ),
  )
  ...
  slots.inject('conversation.input.dock', () =>
    slots.register(
      ...
      (props: { sessionId?: string }) =>
        props.sessionId
          ? React.createElement(MemoryDock, { sessionId: props.sessionId, ctx })
