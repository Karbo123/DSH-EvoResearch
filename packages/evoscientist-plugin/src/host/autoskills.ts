...
export class AutoSkillsService {
  ...
  constructor(readonly config: AutoSkillsConfig) {
    this.file = path.join(config.dataRoot, '.evosci-data', 'autoskills.json')
    this.skillsDir = config.skillsDir ?? path.join(config.dataRoot, '.evosci-data', 'skills')
    this.load()
  }
  ...
  run(proposalId: string): boolean {
    ...
    console.log(`[evosci:autoskills] 运行技能 ${proposal.name}（待接入 dsh-skill 执行）`)
    return true
  }

  /** 更新调度配置（第一版：仅持久化占位，供 WebUI 面板展示）。 */
  saveConfig(config: { enabled?: boolean; mode?: string; cadence?: string }): void {
    const file = path.join(this.config.dataRoot, '.evosci-data', 'autoskills-config.json')
