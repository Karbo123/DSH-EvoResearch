 ...
 *   令牌来自环境变量 EVOSCI_TELEGRAM_TOKEN。
 * - Slack/QQ/微信/飞书：第一版提供适配器骨架（start 提示未配置/待实现），
 *   与 EvoScientist 的 channels/ 目录一一对应，后续按同一接口补齐实现。
 */
...
export class TelegramAdapter implements ChannelAdapter {
  ...
  constructor(token?: string) {
    this.token = token ?? process.env.EVOSCI_TELEGRAM_TOKEN
  ...
  async start(_ctx: Context, onMessage: (message: ChannelMessage) => void): Promise<void> {
    if (!this.token) throw new Error('未设置 EVOSCI_TELEGRAM_TOKEN')
  ...
  private async pollLoop(onMessage: (message: ChannelMessage) => void): Promise<void> {
    while (this.polling && !this.stopRequested) {
      try {
        ...
        // 网络抖动：静默重试（避免刷屏）
        console.error('[evosci:telegram] 轮询失败:', error instanceof Error ? error.message : error)
      }
...
/** 全部内置适配器（与 EvoScientist channels 对应）。 */
export function builtinAdapters(): ChannelAdapter[] {
  return [
    new TelegramAdapter(),
    new PendingAdapter('slack', 'Slack', 'EVOSCI_SLACK_TOKEN'),
    new PendingAdapter('qq', 'QQ', 'EVOSCI_QQ_BOT_ID'),
    new PendingAdapter('wechat', '微信', 'EVOSCI_WECHAT_HOOK'),
    new PendingAdapter('feishu', '飞书', 'EVOSCI_FEISHU_APP_ID'),
    new PendingAdapter('signal', 'Signal', 'EVOSCI_SIGNAL_SOCKET'),
  ]
}
