...
export class SchedulerService {
  ...
  constructor(readonly config: SchedulerConfig) {
    this.file = path.join(config.dataRoot, '.evosci-data', 'scheduler.json')
    this.load()
  }
  ...
  attach(ctx: Context): () => void {
    ...
    this.tickDisposer = timer.interval(() => {
      void this.tick(ctx).catch((error) => {
        console.error('[evosci:scheduler] tick 失败:', error)
      })
    }, 60_000)
    return () => {
      this.tickDisposer?.()
  ...
  private async tick(ctx: Context): Promise<void> {
    ...
    for (const task of this.tasks) {
      ...
      try {
        ...
        console.error(`[evosci:scheduler] 任务 ${task.taskId} 执行失败:`, error)
        task.lastRunAt = Date.now()
      }
      this.save()
    }
