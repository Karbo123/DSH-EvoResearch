/**
 * EvoScientist API 服务：Client（浏览器插件）通过 Typert Gateway 调用的 Remote 方法。
 *
 * 所有方法返回纯 JSON（wire 序列化安全）。命名空间默认 evosci，
 * 对应 EvoScientist WebUI 的 /api/* 路由群（projects/memory/scheduler/channels/autoskills/experts/threads）。
 */
...
/** EvoScientist Remote API。 */
export class EvosciApiService extends TypertRemoteService {
