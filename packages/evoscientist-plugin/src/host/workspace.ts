 ...
 *
 * 对齐 EvoScientist 的项目工作区语义：
 * - 项目目录：<dataRoot>/projects/<name>/，独立 git 仓库；
 * - 项目数据：<dataRoot>/projects/<name>/.evosci-data/（写入项目自身 git exclude）；
...
export class WorkspaceService {
  ...
  /**
   * 创建项目：建目录 + git init + .evosci-data git exclude。
  ...
  /** git init + .evosci-data git exclude（幂等）。 */
  ...
  /** 把 .evosci-data/ 写入 git info/exclude（仅当尚未存在）。 */
  private writeGitExclude(gitDir: string): void {
    ...
    try {
      ...
      if (!content.includes('.evosci-data/')) {
        fs.appendFileSync(excludeFile, '\n# EvoScientist 项目数据（记忆/会话/观测）\n.evosci-data/\n')
      }
    } catch {
      // git 目录不可写时静默（非致命）
    }
