...
export class EvosciDb {
   ...
   */
  static open(file: string, migrations: readonly Migration[]): EvosciDb {
    fs.mkdirSync(path.dirname(file), { recursive: true })
    const db = new DatabaseSync(file)
    db.exec('PRAGMA journal_mode = WAL')
    db.exec('PRAGMA synchronous = FULL')
    db.exec('PRAGMA foreign_keys = ON')
    const handle = new EvosciDb(db, [...migrations].sort((a, b) => a.version - b.version))
    handle.migrate()
    return handle
  }

  /** 打开内存数据库（测试用）。 */
  static openMemory(migrations: readonly Migration[]): EvosciDb {
