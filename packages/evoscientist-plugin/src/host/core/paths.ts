 ...
 *
 * 数据模型与 EvoScientist 一致：
 * - 项目根目录：<dataRoot>/projects/<name>/
 * - 项目数据目录：<dataRoot>/projects/<name>/.evosci-data/
...
/** 每个项目的数据目录名。 */
export const PROJECT_DATA_DIR_NAME = '.evosci-data'
 ...
 * 与 EvoScientist 的 _project_slug 回退语义一致。
 */
...
/** 项目科研记忆库路径（.evosci-data/memories/research_memory.db）。 */
...
/** 项目 Observations 目录（.evosci-data/memories/observations）。 */
 ...
 * 与 EvoScientist paths.validate_webui_workspace 语义一致。
 */
