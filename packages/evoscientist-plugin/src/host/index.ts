/**
 * EvoScientist Host 插件入口。
 *
 * 组装全部科研能力：
 * - WorkspaceService：科研项目工作区（projects/<name>/.evosci-data 隔离）；
 ...
 * - EvosciApiService：Client 可调用的 Remote API；
 * - 斜杠命令：/project /memory /schedule /channel /expert /autoskills。
 *
 * 配置（settings.yaml 的 evosci 段，或环境变量）：
 * ```yaml
 * evosci:
 *   dataRoot: D:\evoscientist      # 部署根目录（projects/ 所在目录），默认 $EVOSCI_DATA_ROOT 或 cwd
...
import { EvosciApiService, type HostServices } from './api.js'
import { registerCommands } from './commands.js'
import { listProjects } from './core/paths.js'

/** 插件配置（settings 的 evosci 段合并环境变量）。 */
export interface EvosciPluginConfig {
  /** 部署根目录。 */
...
/** 从 settings/env 解析配置。 */
function resolveConfig(ctx: Context): EvosciPluginConfig {
  const settings = ctx.get('settings') as { get?: (ns: string) => unknown } | undefined
  const fromSettings = (settings?.get?.('evosci') ?? {}) as Partial<EvosciPluginConfig>
  const fromEnv: { dataRoot?: string } = {}
  if (process.env.EVOSCI_DATA_ROOT) fromEnv.dataRoot = process.env.EVOSCI_DATA_ROOT
...
function apply(ctx: Context): void {
  ...
  console.log(`[evosci] host 插件激活（dataRoot: ${dataRoot}）`)
  ...
  channels.setDeliver(async (message: ChannelMessage) => {
    ...
    const cwd = projectName
      ...
      : dataRoot
    return deliverToAgent(ctx, `[${message.senderName ?? message.senderId} 经 ${message.chatId} 通道] ${message.text}`, cwd, 'evosci:channel')
  })
  ...
  if (config.autoStartChannels) {
    void channels.startAll().catch((error) => {
      console.error('[evosci] 自动启动通道失败:', error)
    })
  }
