 ...
 *
 * 表结构对齐 D:\EvoScientist\EvoScientist\memory\research\store.py 的核心语义：
...
import { EvosciDb, type Migration, cleanForIndex } from '../core/db.js'
import {
  type TurnRecord,
  type TurnStatus,
...
export class ResearchMemoryStore {
  readonly db: EvosciDb

  private constructor(db: EvosciDb) {
    this.db = db
  }
  ...
  static open(memoryDir: string): ResearchMemoryStore {
    ...
    return new ResearchMemoryStore(EvosciDb.open(file, RESEARCH_MEMORY_MIGRATIONS))
  }

  /** 内存库（测试用）。 */
  static openMemory(): ResearchMemoryStore {
    return new ResearchMemoryStore(EvosciDb.openMemory(RESEARCH_MEMORY_MIGRATIONS))
  }
